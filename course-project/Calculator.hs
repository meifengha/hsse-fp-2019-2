...
resolve_prefix_minus (a:(Operator Minus):xs) =  resolve_prefix_minus $ a : (Operator Plus)  : (Operand (-1)) : (Operator Multiply) : xs
resolve_prefix_minus ((Operator Minus):xs) =  (Operand (-1)) : (Operator Multiply) : resolve_prefix_minus xs

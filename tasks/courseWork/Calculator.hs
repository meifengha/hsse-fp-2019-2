...
displayExpr xs = do writeAt (2,2) "               \n"
                     if length xs < boxWidth - 2 then
                     writeAt (2,2) $ replicate (boxWidth - length xs - 2) ' ' ++ xs
                     else
                     writeAt (2,2) $ ">" ++ drop (length xs - boxWidth + 3) xs
...
eval n xs =  case parse (expr n) xs of
           [(e,"")] -> case evalExpr e of
                        (Just a) -> if n == 0 then
                                   displayAns n xs (show a)
                                   else
                                   displayAns n xs (decToHex a)
...
displayAns n pxs xs = do writeAt (3,2) "               \n"
                         if length xs < boxWidth - 2 then
                         writeAt (3,2) $ replicate (boxWidth - length xs - 2) ' ' ++ xs
                         else
                         writeAt (3,2) $ ">" ++ drop (length xs - boxWidth + 3) xs
                         c <- getCh
                         process n c pxs

decToHex :: Int -> String
decToHex 0 = []
decToHex n = if (n >= 0 ) then
             decToHex (n `div` 16) ++ [hexChar $ n `mod` 16]
             else
             '-' : decToHex (-n)

hexToDec :: String -> Int
hexToDec [] = 0
hexToDec (x:xs) =  if (x == '-' ) then
                   -hexToDec xs
                   else
                   16 ^ length xs * hexInt x + hexToDec xs

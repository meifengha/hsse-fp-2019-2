...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    val list: List[Occurrences] = occurrences.map(x => (for (i <- 1 until (x._2 + 1)) yield (x._1, i)).toList)
    list.foldRight(List[Occurrences](Nil))((x, y) => y ++ (for (i <- x; j <- y) yield i :: j))
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def anagrams(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(Nil)
        ...
        combination <- combinations(occurrences) if dictionaryByOccurrences.keySet(combination)

...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    ...
    word.grouped(c => c).map { case (ch, str) => (ch, str.length) }.toList.sortBy(_._1)
  }

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = {
    def sentenceOccurrencesIterator(s: Sentence): Occurrences = {
      if (s.isEmpty) Nil
      else wordOccurrences(s.head) ++ sentenceOccurrencesIterator(s.tail)
    }
    sentenceOccurrencesIterator(s)

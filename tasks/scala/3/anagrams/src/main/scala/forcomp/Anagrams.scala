...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy(identity).map{ case (k, v) => (k, v.length) }.toList.sortBy(_._1)
   ...
   */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = x.flatMap{
    case (cx, ox) => y.find{ case (cy, oy) => cy == cx } match {
        case Some((_, o)) if o == ox => None
        case Some((c, o)) => Some (c, ox - o)
        case None => Some(cx, ox)
      }
  }

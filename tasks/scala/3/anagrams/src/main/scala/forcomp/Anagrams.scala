...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.groupBy(e => e.toLower).toList.sorted map (e => (e._1, e._2.length))
  }

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = {
    wordOccurrences(s.foldLeft("")((c1,c2) => c1 + c2))
  }
   ...
   */
  def sentenceAnagrams(sent: Sentence): List[Sentence] = {
    def iter(occur: Occurrences): List[Sentence] = {
      if (occur.isEmpty) List(Nil)
      else for
        {
          comb <- combinations( occur )
          word <- dictionaryByOccurrences getOrElse (comb, Nil)
          sent <- iter( subtract(occur,wordOccurrences(word))
        )
        if !comb.isEmpty
      } yield word :: sent
    }

    iter( sentenceOccurrences(sent) )
  }

...
getFilePath :: Maybe String -> IO (Maybe String)
getFilePath Nothing = putStrLn "Неверные значения.Пример использования: ./Main Mandelbrot.png" >> return Nothing
getFilePath (Just fileName) = putStrLn ("Генерируется " ++ fileName) >> return (Just fileName)

savePNG :: Maybe String -> DynamicImage -> IO ()
savePNG Nothing _ = return ()
savePNG (Just fileName) image = savePngImage fileName image
